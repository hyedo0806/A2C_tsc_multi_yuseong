...
class A2C:
    ...
    def _init_scheduler(self, model_config):
        lr_init = model_config.getfloat('lr_init')                                      ## 5e-4
        lr_decay = model_config.get('lr_decay')                                     ## constant
        beta_init = model_config.getfloat('entropy_coef_init')        ## 0.01
        beta_decay = model_config.get('entropy_decay')                  ## constant

        if lr_decay == 'constant':
            self.lr_scheduler = Scheduler(lr_init, decay=lr_decay)        ## agents\utils.py class Scheduler
        else:
    ...
    def _init_train(self, model_config):
        ...
        self.trans_buffer = OnPolicyBuffer(gamma)                              ## agents\utils.py class OnPolicyBuffer
...
class IA2C(A2C):
    def __init__(self, n_s_ls, n_a_ls, n_w_ls, total_step,
        ...
        if total_step:
            # training
            self.total_step = total_step
            self._init_scheduler(model_config)   ##A2C
            self._init_train(model_config)            ##IA2C
        self.sess.run(tf.global_variables_initializer())
