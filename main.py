...
from envs.yuseong import YuseongEnv, YuseongController
from agents.models import A2C, IA2C, MA2C, IQL
from utils import (Counter, Trainer, check_dir, copy_file, find_file,
                   init_dir, init_log, init_test_flag,
                   plot_evaluation, plot_train)

def parse_args():

    default_base_dir = 'D:\PycharmProjects\mutli_intersaction_A2C_211223\large_grid'
    default_config_dir = 'D:\PycharmProjects\mutli_intersaction_A2C_211223\config\config_yuseong.ini'
...
def init_env(config, port=0, naive_policy=False):
    if config.get('scenario') == 'yuseong':
        if not naive_policy:
            print("not naive policy")
            return YuseongEnv(config)
        else:
            env = YuseongEnv(config)
            policy = YuseongController(env.node_names)
            return env, policy
    '''    
...
def train(args):
    ...
    copy_file(config_dir, dirs['data'])  ## 메모장 오픈됨.
    config = configparser.ConfigParser()
    config.read(config_dir)
    in_test, post_test = init_test_flag(args.test_mode)
    print("args.test_mode ::: ", args.test_mode)
    print( "test :::: " ,in_test, post_test)

    # init env
    ## VISSIM 환경 설정
    env = init_env(config['ENV_CONFIG'])
    print("env : ", env)
    print("config : ", config['ENV_CONFIG'])
    ...
    if env.agent == 'ia2c':
        model = IA2C(env.n_s_ls, env.n_a_ls, env.n_w_ls, total_step, config['MODEL_CONFIG'], seed=seed)
        print("model : ", model)
    '''
    if env.agent == 'ia2c':
